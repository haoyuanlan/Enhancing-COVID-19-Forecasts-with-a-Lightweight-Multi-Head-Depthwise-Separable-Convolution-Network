...
def country_list_smooth(data, attributes_num, ratio_list):
    data_dict = {}
    data_keys = list(data.keys())
    for i in data_keys:
        data_dict[i] = country_smooth(data[i], attributes_num, ratio_list)
    return data_dict
...
class C_L(api.Model):
    def __init__(self):
        super().__init__()
        self.cov = api.layers.Conv1D(16, kernel_size=3, padding='same', activation='relu')
        self.maxpooling = api.layers.MaxPool1D()

        self.lstm_1 = api.layers.LSTM(32, return_sequences=True)
        self.lstm_2 = api.layers.LSTM(32, return_sequences=False)
        self.dense_1 = api.layers.Dense(7)
...
def single_step_prdition(model, data_scaler, train_X, train_y, test_X, test_y, org_data, regre_len, block = True):
    ...
    return true_data, pred_data, pred, y, rmse(y, pred)
...
if __name__ == "__main__":
    ...
    num = 820
    attributes = ['New_deaths']
    file = ['pl']
    file_name = [i + '.xlsx' for i in file]
    dir_name = os.path.join('datasets', 'Case_3')

    data = get_data(dir_name, file_name, num, attributes, )

    attributes_num = [0]
    ratio_list = np.ones(7)/7
    smooth_data = country_list_smooth(data, attributes_num, ratio_list)
    ...
    for i in range(run_times):
        # train_y = train_y.reshape(train_y.shape[0], regre_len)
        # model = L_C()
        ...
        org_data = smooth_data[file_name[0]][attributes[0]]

        true_data, pred_data, pred_on_test, label_on_test, rmse_value = single_step_prdition(model, data_scaler, train_X, train_y, test_X, test_y, org_data, regre_len, block=block)

        rmse_list = np.append(rmse_list, rmse_value)
    ...
    rmse_list = np.append(rmse_list, np.mean(rmse_list))

    figure, axs = plt.subplots(1, 2, constrained_layout=True)
    figure.suptitle('Metric')

    x = np.arange(1, len(rmse_list))
    x = np.append(x, 'average')

    ax_1 = axs[0]
    ax_1.set_title('rmse')
    bar_1 = ax_1.bar(x, rmse_list)
