...
if __name__ == "__main__":
    ...
    num = 250
    attributes = ['Cumulative_cases']
    file = ['brazil']
    file_name = [i + '.xlsx' for i in file]
    dir_name = os.path.join('datasets', 'Case_2')

    data = get_data(dir_name, file_name, num, attributes, )
    smooth_data = seven_data_smooth(data, file, attributes)

    # brazil, start_day = 0, nnum = 212
    start_day = 0
    nnum = 212
    smooth_data = split_data(smooth_data, start_day, end=nnum)
    ...
    data_visualization(smooth_data, suptitle=training_suptitle, title=training_figure_title,
    ...
    scaled_training_data, data_scaler, feature_scaler = training_normalization(smooth_data,
    ...
    for i in range(run_times):
        ...
        org_data = smooth_data[file_name[0]][attributes[:1]]
        ...
        if if_cumulative_data:
            ...
            daily_cases_label = new_cases.iloc[:, :1]
            daily_cases_label.columns = ['daily cases']

            daily_predicted_value = new_cases.iloc[:, 1:]
            daily_predicted_value.columns = ['daily predicted value']

            final_data = pd.concat([daily_cases_label, daily_predicted_value], axis=1)

            final_data.to_excel(writer, sheet_name='Sheet3')

            daily_mae = mae(np.array(daily_cases_label)[-18:], np.array(daily_predicted_value)[-18:])
            daily_mae_list = np.append(daily_mae_list, daily_mae)
    ...
    daily_mae_list = np.append(daily_mae_list, np.mean(daily_mae_list))
    figure, axs = plt.subplots(1, 2, constrained_layout=True)
    figure.suptitle('Metric')

    x = np.arange(1, len(daily_mae_list))
    x = np.append(x, 'average')

    ax_1 = axs[0]
    ax_1.set_title('daily_mae')
    bar_1 = ax_1.bar(x, daily_mae_list)
