...
def training_normalization(data, suptitle, title, data_label, visualization=True, max_figure_num=2, block = True):
...
def seven_data_smooth(data, country_list, attributes):
    data_dict = {}
    arr = np.array([])
    for n in range(len(country_list)):
        key = list(data.keys())[n]
        country_data = data[key]
        for i in range(len(country_data)):
            if i < 3:
                arr = np.append(arr, country_data.iloc[i])
            elif i < len(country_data) - 3:
                value = np.mean(country_data.iloc[i - 3:i + 4])
                arr = np.append(arr, value)
            else:
                arr = np.append(arr, country_data.iloc[i])

        smoothed_arr = pd.DataFrame(arr, columns=attributes)
        data_dict[key] = smoothed_arr
    return data_dict



def data_visualization(data, suptitle, title, data_label, cols=4, rows=4, max_figure_num=2, block=True):
...
class CNN_LSTM(api.Model):
    def __init__(self):
        super().__init__()
        self.cnn_1 = api.layers.Conv1D(32, kernel_size=2, activation='relu')
        self.cnn_2 = api.layers.Conv1D(45, kernel_size=2, activation='relu')
        self.cnn_3 = api.layers.Conv1D(64, kernel_size=2, activation='relu')

        self.lstm_1 = api.layers.LSTM(175, return_sequences=True)
        self.lstm_2 = api.layers.LSTM(175, return_sequences=True)
        self.lstm_3 = api.layers.LSTM(175)

        self.dense_1 = api.layers.Dense(3)
        self.dense_2 = api.layers.Dense(1)
...
def single_step_prdition(model, data_scaler, train_X, train_y, test_X, test_y, org_data, regre_len, block = True):
    ...
    return true_data, pred_data, pred, y, mae(y, pred)
...
def split_data(data, start, end):
    ...
    key_list = list(data.keys())
    for i in key_list:
        pd_data = data[i].iloc[start:start+end]
        data_dict[i] = pd_data
    return data_dict


def get_cumulatives(first_data, daily_data_list):
    cumulative_list = []
    for i in range(len(daily_data_list)):
        if i == 0:
            value = first_data + daily_data_list[0]
        else:
            value = cumulative_list[-1] + daily_data_list[i]
        cumulative_list.append(value)
    return np.array(cumulative_list)


def statistic_test(baseline_errors, target_errors, alpha, confidence_level):
    # Calculate differences: D = baseline - cdscnet
    differences = baseline_errors - target_errors
    ...
    target_mean = np.mean(target_errors)
    improvement_percent = (baseline_mean - target_mean) / baseline_mean * 100

    print("Basic statistics:")
    print(f"Sample size (folds): n= {n}")
    print(f"Mean difference (baseline - CDSCnet): {mean_diff:.2f}")
    print(f"Standard deviation of differences: {std_diff:.2f}")
    print(f"Standard error of differences: {sem_diff:.2f}")
    print(f"Baseline model mean MAE: {baseline_mean:.2f}")
    print(f"CDSCnet mean MAE: {target_mean:.2f}")
    print(f"Relative improvement of CDSCnet: {improvement_percent:.1f}%")
    print()


    t_stat, p_value = stats.ttest_rel(baseline_errors, target_errors)

    print("Paired t-test results:")
    print(f"t-statistic: t({n - 1})= {t_stat:.4f}")
    print(f"p-value: p = {p_value:.6f}")
    print()

    # ============================================
    # 4. Interpretation and significance judgment
    # ============================================
    print(f"Significance level: α = {alpha}")

    if p_value < alpha:
        print("Conclusion: Reject the null hypothesis (H0)")
        print("Interpretation: The difference in performance between CDSCnet and the baseline is statistically significant")

        if mean_diff > 0:
            print("Direction: CDSCnet significantly outperforms the baseline model")
        else:
            print("Direction: Baseline model significantly outperforms CDSCnet")
    else:
        print("Conclusion: Fail to reject the null hypothesis (H0)")
        print("Interpretation: There is insufficient evidence to conclude a significant difference between the two models")
    ...
    print(f"{confidence_level*100:.0f}% CI for mean difference ({mean_diff:.2f}): [{ci_lower:.2f}, {ci_upper:.2f}]")
    print(f"Interpretation: We are {confidence_level*100:.0f}% confident that the true difference in errors between the baseline and CDSCnet lies within [{ci_lower:.2f}, {ci_upper:.2f}]")

    if ci_lower > 0 < ci_upper:
        print("Since the confidence interval lies entirely above zero, this further supports the conclusion that CDSCnet significantly outperforms the baseline model")
    elif ci_lower < 0 > ci_upper:
        print("Since the confidence interval lies entirely below zero, this further supports the conclusion that the baseline model significantly outperforms CDSCnet")
    else:
        print("Since the confidence interval contains zero, this is consistent with a non-significant result")
...
if __name__ == "__main__":
    ...
    num = 250
    forecast_day = 7
    num_other = num + forecast_day

    file = ['spain']
    attributes = ['Daily_cases']
    file_name = [i + '.xlsx' for i in file]
    dir_name = os.path.join('datasets', 'Case_2')

    data = get_data(dir_name, file_name, num, attributes, )
    data_other = get_data(dir_name, file_name, num_other, attributes, )

    cumulative_attributes = ['Cumulative_cases']
    cumulative_data = get_data(dir_name, file_name, num_other, attributes=cumulative_attributes, )

    # us/spain = start_day = 0, nnum =148
    start_day = 0
    nnum = 148

    data = split_data(data, start_day, end=nnum)

    cumulative_data = split_data(cumulative_data, start_day, end=nnum)[file_name[0]].values
    ...
    data_visualization(data, suptitle=training_suptitle, title=training_figure_title,
                       data_label=training_label, max_figure_num=max_figure_num, block=block)
    ...
    train_X, train_y, test_X, test_y = data_format(scaled_training_data[0], num_steps, target_day, attributes_len,
                                                   regre_len, test_ratio=test_ratio)
    ...
    for i in range(run_times):
        ...
        # model = DSC_GRU(target_day, regre_len)
        # model = CNN_LSTM()
        ...
        true_data, pred_data, pred_on_test, label_on_test, mae_value = single_step_prdition(model, data_scaler, train_X, train_y, test_X, test_y, org_data, regre_len, block=block)


        first_data = cumulative_data[-18]
        pred_cumulatives = get_cumulatives(first_data, pred_on_test)
        label_cumulatives = get_cumulatives(first_data, label_on_test)
        cumulative_mae_list.append(mae(label_cumulatives, pred_cumulatives))
        ...
        data_to_excel_test = np.concatenate([label_cumulatives, pred_cumulatives], axis=-1)
    ...
    cumulative_mae_list = np.append(cumulative_mae_list, np.mean(cumulative_mae_list))

    figure, axs = plt.subplots(1, 2, constrained_layout=True)
    figure.suptitle('Metric')

    x = np.arange(1, len(cumulative_mae_list))
    x = np.append(x, 'average')

    ax_1 = axs[0]
    ax_1.set_title('mae')
    bar_1 = ax_1.bar(x, cumulative_mae_list)
    ax_1.bar_label(bar_1, fmt='%d')
    ...
    '''
    Pleaase attention: the statistical significance was obtained based on 10 independent experiments of CDSCnet and the reproduced CNN-LSTM. 
    The statistical significance data used in the paper are provided below:
    
    the mae of reproduced CNN-LSTM in Spain ：2556，2181，2485，944，3224，1021，1904，1636，2925，5093
    the mae of CDSCnet in Spain：210，195，678，1116，959，916，810，1337，878，889
    
    For further verification, select the corresponding model, repeat the  experiments, 
    and input the recorded experimental results into the statistic_test function for calculation
    
    '''
    #
    # baseline_errors = np.array([2556, 2181, 2485, 944, 3224, 1021, 1904, 1636, 2925, 5093])
    # cdscnet_errors = np.array([210, 195, 678, 1116, 959, 916, 810, 1337, 878, 889])
    #
    # statistic_test(baseline_errors, cdscnet_errors, alpha, confidence_level)
